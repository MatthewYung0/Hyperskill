...
public class Main {
    ...
    public static String createRandomNumber(String codeLength) {
        ...
        String number = "";
        // Condition variable for checking if number is valid
        boolean isValidNumber = false;

        Random random = new Random();
        int minimum = (int) Math.pow(10, Integer.parseInt(codeLength) - 1);

        initialiseNumberTemplate();

        while (!isValidNumber) {
            // Generating a random number and storing in randomNumber variable
            String randomNumber = String.valueOf(minimum + random.nextInt(9 * minimum));
