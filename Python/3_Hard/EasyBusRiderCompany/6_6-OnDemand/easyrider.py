...
def check_valid_time(data_, bus_lines_):
    wrong_times = {}
    for bus_line in bus_lines_:
        previous_time = 0
        for route in data_:
            if route["bus_id"] == bus_line:
                current_time = get_minutes(route["a_time"])
                if current_time >= previous_time:
                    previous_time = current_time
                else:
                    key = route["bus_id"]
                    value = route["stop_name"]
                    wrong_times[key] = value
                    break
    return wrong_times
def get_minutes(time):
    return int(time[:2]) * 60 + int(time[3:])

def check_on_demand_stops(data_):
    invalid_stops = []
    valid_on_demand_stops = ["Fifth Avenue", "Sesame Street"]
    for item in data_:
        if item["stop_type"] == "O":
            if item["stop_name"] not in valid_on_demand_stops:
                invalid_stops.append(item["stop_name"])
    return list(dict.fromkeys(sorted(invalid_stops)))
...
def main():
    ...
    # wrong_times = check_valid_time(bus_data, bus_lines)
    # print("Arrival time test:")
    # if len(wrong_times) == 0:
    #     print("OK")
    # else:
    #     for key, value in wrong_times.items():
    #         print("bus_id line {}: wrong time on station {}".format(key, value))

    invalid_stops = check_on_demand_stops(bus_data)
    print("On demand stops test:")
    if len(invalid_stops) == 0:
        print("OK")
    else:
        print("Wrong stop type: {}".format(invalid_stops))
